...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Bookstore Sederhana</title>
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex">
                            <div class="flex-shrink-0 flex items-center">
                                <a href="{{ route('home') }}" class="font-bold text-xl">Bookstore</a>
                            </div>
                        </div>
                                ...
                                <div class="space-x-4">
                                    ...
                                    @else
                                        ...
                                        @if (Route::has('register'))
                                            <a href="{{ route('register') }}" class="font-medium text-gray-600 hover:text-gray-900 ml-4">Register</a>
                                        @endif
                                    @endauth
                                </div>
                            @endif
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                @forelse ($books as $book)
                                    <a href="{{ route('books.show', $book->id) }}" class="block group">
                                        <div class="bg-white overflow-hidden shadow-md rounded-lg h-full flex flex-col transform transition-transform duration-300 group-hover:-translate-y-2">
                                ...
                                @empty
                                    <div class="col-span-full text-center text-gray-500 py-10">
                                        <p>Buku yang Anda cari tidak ditemukan atau belum ada buku yang tersedia.</p>
                                    </div>
                                @endforelse
                            </div>
